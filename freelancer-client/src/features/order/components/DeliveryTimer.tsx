...
const DeliveryTimer: FC<IModalProps> = ({ order, authUser }): ReactElement => {
  ...
  return (
    <>
      ...
      <div className="rounded mb-6 flex flex-col gap-4 bg-blue-700/20 px-4 py-3 border border-blue-700/30">
        ...
        {authUser?.username === order?.sellerUsername && (
          <div className="flex w-full cursor-pointer flex-col gap-4">
            <Button
              className="w-full rounded bg-green-500/90 px-4 py-2 text-center text-sm font-bold text-white hover:bg-green-500 focus:outline-none md:text-base"
              label={`Deliver ${!order?.delivered ? 'Now' : 'Again'}`}
              onClick={() => setDisplayModal({ ...displayModal, deliverWork: !displayModal.deliverWork })}
            />
            {!order?.delivered && (
              <div
                className="mb-2 text-center text-sm underline md:text-base"
