      ...
      <div className="flex flex-wrap">
        <div className="order-last w-full p-4 lg:order-first lg:w-2/3">
          ...
          {order && order.buyerUsername === authUser.username && (
            <div className="rounded mt-4 flex flex-col justify-between bg-blue-700/20 md:flex-row border border-blue-700/30">
        ...
        <div className="w-full p-4 lg:w-1/3">
          {Object.keys(order).length > 0 ? (
            <>
              ...
              <div className="bg-blue-700/20 rounded border border-blue-700/30">
                <div className="mb-2 flex flex-col border-b px-4 pb-4 pt-3 md:flex-row">
                  <img className="h-12 w-20 object-cover pt-1" src={order?.gigCoverImage} alt="Gig Cover Image" />
                  <div className="flex flex-col">
                    <h4 className="mt-2 text-sm font-bold md:mt-0 md:pl-4">{order.offer.gigTitle}</h4>
                    <span
                      className={`status mt-1 w-24 rounded px-[3px] py-[3px] text-xs font-bold uppercase md:ml-4 ${order.status.replace(
                        / /g,
                        ''
                      )}`}
                    >
                  ...
                  <li className="flex justify-between px-4 pb-2 pt-2">
                    ...
                    {authUser.username === order.sellerUsername ? (
                      ...
                      <span className="text-sm font-bold text-green-500">
                        <Link
                          to={`/seller_profile/${lowerCase(`${order.sellerUsername}`)}/${order.sellerId}/view`}
