...
const Checkout: FC = (): ReactElement => {
  ...
  const totalFee: number = offer.price + serviceFee;
  ...
  const createBuyerOrderIntent = async (): Promise<void> => {
    try {
      ...
      // from server/01-gateway-service/src/controllers/order/create.ts
      ...
      <div className="flex flex-wrap">
        <div className="w-full p-4 lg:w-2/3 order-last lg:order-first">
          <div className="border border-grey rounded">
            <div className="text-xl font-medium mb-2 pt-3 pb-4 px-4">
              <span>Payment</span>
            </div>
            <div className="text-base xl:text-lg font-medium mb-2 pt-2 pb-2 px-4">
              <span>Visa </span>
              <span> number: 4242424242424242 </span>
        ...
        <div className="w-full p-4 lg:w-1/3">
          <div className="border border-grey mb-8 rounded">
            <div className="pt-3 pb-4 px-4 mb-2 flex flex-col border-b md:flex-row">
              ...
              <h4 className="font-bold text-sm mt-2 md:pl-4 md:mt-0">{state.title}</h4>
            </div>
            <ul className="list-none mb-0">
              ...
              <li className="flex justify-between px-4 py-4">
                ...
                <span className="text-sm md:text-base font-semibold">${totalFee.toFixed(2)}</span>
              </li>
            </ul>
          </div>
        </div>
