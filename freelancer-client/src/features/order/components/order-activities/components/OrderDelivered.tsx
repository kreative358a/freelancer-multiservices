...
const OrderDelivered: ForwardRefExoticComponent<Omit<IOrderDeliveredProps, 'ref'> & RefAttributes<HTMLDivElement>> = forwardRef(
  (_, ref) => {
    ...
    const chatSeller: IChatSellerProps = {
      ...
      //       profilePicture: `${order?.sellerImage !== null ? order?.sellerImage : 'https://placehold.co/330x220?text=Profile+Image'}`,
      responseTime: 1
    };
    ...
    const downloadOrderFile = async (url: string, fileName: string) => {
      try {
        ...
        console.log('downloadOrderFile error: ', error);
    ...
    return (
      <>
          ...
          <div className="flex rounded-[4px] bg-blue-700/20 px-4 py-3 border border-blue-700/30">
            <div className="w-full">
              <div className="flex gap-4">
                ...
                <div className="border-grey w-full border-b pb-5" ref={ref}>
                  <div className="mt-2 flex items-center justify-between font-medium text-gray-600 dark:text-gray-400">
                    <div className="flex gap-2">
                      ...
                      <p className="flex self-center text-sm font-normal italic text-teal-600 dark:text-teal-400">
                  ...
                  {orderDeliveredModal.delivery && (
                    <div className="my-3 flex flex-col">
                      <div className="relative overflow-x-auto">
                        <div className="border-grey w-full rounded border text-left text-sm xl:text-base">
                          <div className="border-grey border-b bg-blue-400/20 dark:text-teal-400 text-teal-800 py-3 uppercase">
                            <span className="px-5 font-semibold">Deliver{order?.deliveredWork.length > 1 ? 'ies' : 'y'}</span>
                          </div>
                          {order.deliveredWork.map((work: IDeliveredWork) => (
                            <div
                              key={uuidv4()}
                              className="border-grey flex w-full flex-col items-center space-x-4 border-b px-5 pt-2 last:border-none md:flex-row"
                            >
                              ...
                              <div className="w-full text-sm dark:text-white xl:text-base">
                                <div className="flex justify-between text-sm font-bold text-slate-600 dark:text-slate-400 md:text-base">
                                  <span>{authUser?.username === order.buyerUsername ? `${order.sellerUsername}'s message` : 'Me'}</span>
                                </div>
                                <div className="flex flex-col justify-between text-slate-800 dark:text-slate-200">
        ...
        {order?.delivered && order?.deliveredWork && order?.deliveredWork.length > 0 && (
          <div className="flex rounded-[4px] bg-blue-700/30 px-4 py-1 border border-blue-700/40">
            <div className="w-full">
              <div className="flex gap-4">
                <div>
                ...
                <div className="border-grey w-full border-b pb-6">
                  <div className="mt-2 flex items-center gap-2 font-medium text-gray-600 dark:text-gray-400">
                      ...
                      <div className="relative overflow-x-auto">
                        <div className="text-left text-sm text-gray-600 dark:text-gray-400">
                          <div className="border-grey flex w-full cursor-pointer flex-col items-center space-x-4 border-b md:flex-row">
                            <div className="w-full text-sm dark:text-white xl:text-base">
                              <div className="flex flex-col justify-between text-slate-700 dark:text-slate-300">
                                  ...
                                  <Button
                                    ...
                                    onClick={() => {
                                      setApprovalModalContent({
                                        ...
                                        btnColor: 'bg-sky-500/90 hover:bg-sky-500'
                                      });
