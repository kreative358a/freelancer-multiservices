...
import { lowerCase, replaceSpacesWithDash } from 'src/shared/utils/utils.service';
...
const Requirement: FC = (): ReactElement => {
  ...
  const orderId = `FO${generateRandomNumber(11)}`;
  const invoiceId = `FI${generateRandomNumber(11)}`;
  ...
  const startOrder = async (): Promise<void> => {
    try {
      ...
      const order: IOrderDocument = {
        ...
        // sellerImage: `${gigRef?.current?.profilePicture ? gigRef?.current?.profilePicture : 'https://placehold.co/330x220?text=Profile+Image'}`,
        // sellerImage: `${gigRef?.current?.profilePicture}` ?? 'https://placehold.co/330x220?text=Profile+Image',
        ...
        buyerId: `${buyer._id}`,
        buyerUsername: `${buyer.username}`,
        // buyerImage: `${buyer.profilePicture}` ?? 'https://placehold.co/330x220?text=Profile+Image',
        // buyerImage: `${buyer.profilePicture ? buyer.profilePicture : 'https://placehold.co/330x220?text=Profile+Image'}`,
    ...
    <div className="container mx-auto lg:h-screen">
      <div className="flex flex-wrap">
        <div className="order-last w-full p-4 lg:order-first lg:w-2/3">
          <div className="mb-4 flex w-full flex-col flex-wrap  p-4">
            <span className="text-base font-bold lg:text-xl">Thank you for your purchase</span>
          ...
          <div className="border-grey border rounded">
            <div className="mb-3 px-4 pb-2 pt-3">
              <span className="mb-3 text-base font-medium md:text-lg lg:text-xl">Any information you would like the seller to know?</span>
            ...
            <div className="flex flex-col px-4 pb-4">
              ...
              />
              <Button
                className="mt-3 rounded bg-sky-500/90 hover:bg-sky-500 px-6 py-3 text-center text-sm font-bold text-white focus:outline-none md:px-4 md:py-2 md:text-base"
                label="Start Order"
                onClick={startOrder}
              />
            </div>
        ...
        <div className="w-full p-4 lg:w-1/3">
          <div className="border-grey mb-8 border rounded">
            ...
            <ul className="mb-0 list-none">
              <li className="border-grey flex border-b px-4 pb-3 pt-1">
                <Link
                  to={`/gig/${lowerCase(`${gigRef?.current?.username}`)}/${replaceSpacesWithDash(offer.gigTitle)}/${gigRef?.current?.sellerId}/${gigId}/view`}
