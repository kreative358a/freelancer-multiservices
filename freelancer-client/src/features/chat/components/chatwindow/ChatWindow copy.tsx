  ...
  const sendMessage = async (event: FormEvent): Promise<void> => {
    ...
    try {
      ...
      const messageBody: IMessageDocument = {
        ...
        senderPicture: `${authUser?.profilePicture !== 'undefined' ? authUser?.profilePicture : 'https://placehold.co/330x220?text=Profile+Image'}`,
        // senderPicture: `${authUser?.profilePicture}`
        receiverUsername: receiverRef?.current?.username,
        receiverPicture: `${receiverRef?.current?.profilePicture !== 'undefined' ? receiverRef?.current?.profilePicture : 'https://placehold.co/330x220?text=Profile+Image'}`,
        ...
        <div className="flex min-h-full w-full flex-col">
          <div className="border-grey flex w-full flex-col border-b px-5 py-0.5 ">
            {receiverUsername.current === receiverRef?.current?.username ? (
              <>
                <div className="text-lg font-semibold capitalize">{firstLetterUppercase(`${username}`)}</div>
              ...
              <>
                <div className="py-2.5 text-lg font-semibold capitalize">{firstLetterUppercase(`${username}`)}</div>
                <span className="py-2.5s text-xs font-normal"></span>
              </>
            )}
          </div>
          <div className="relative h-[100%]">
            <div className="relative border-b border-grey flex h-[98%] w-screen grow flex-col overflow-scroll pt-4 mb-2" ref={scrollRef}>
              {chatMessages.map((message: IMessageDocument) => (
                <div key={uuidv4()} className="mb-2">
                  <div className="flex w-full cursor-pointer items-center space-x-4 px-5 py-2 hover:bg-[#f5fbff] dark:hover:bg-[#172c3a]">
                    <div className="flex self-start">
                      <img
                        className="h-10 w-10 object-cover rounded-full"
                        src={
                          message.senderPicture !== 'undefined' ? message.senderPicture : 'https://placehold.co/330x220?text=Profile+Imag'
                        }
                    ...
                    <div className="w-full text-sm dark:text-white xl:text-base">
                      <div className="flex gap-x-2 pb-1 font-bold text-slate-700 dark:text-slate-300">
                        <span>{message.senderUsername}</span>
                        <span className="mt-1 self-center text-xs font-normal xl:text-sm">
            ...
            {showImagePreview && (
              <>
                <div className="flex flex-row gap-4">
                  ...
                  <Button
                    className="rounded bg-sky-500/90 hover:bg-sky-500 px-6 py-3 text-center text-sm font-bold text-white focus:outline-none md:px-4 md:py-2 md:text-base h-10 absolute bottom-2 right-2"
                    disabled={false}
