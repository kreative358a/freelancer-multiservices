  ...
  return (
    <>
      {isMessageLoading && !chatMessages ? (
        ...
        <div className="border-grey fixed bottom-0 left-2 right-2 h-[400px] max-h-[500px] w-auto border bg-white md:left-8 md:h-96 md:max-h-[500px] md:w-96 rounded">
          <div className="border-grey flex items-center space-x-4 border-b px-5 py-2">
            <img
              ...
              alt="profile image"
            />
            <div className="w-full font-medium text-slate-700 dark:text-slate-300">
              <div className="flex w-full cursor-pointer justify-between text-sm font-bold text-slate-700 dark:text-slate-300 md:text-base">
          ...
          <div className="h-[500px] overflow-y-scroll md:h-full">
            <div className="my-2 flex h-[280px] flex-col overflow-y-scroll px-4 md:h-[72%]" ref={scrollRef}>
              {chatMessages.map((msg: IMessageDocument) => (
                ...
                >
                  <img
                    src={buyer.profilePicture !== 'undefined' ? buyer.profilePicture : 'https://placehold.co/330x220?text=Profile+Image'}
                    className={`h-8 w-8 rounded-full object-cover bg-gray-500 ${msg.senderUsername !== buyer.username ? 'hidden' : ''}`}
                    alt="profile image"
                  />
                  <p
          ...
          <form onSubmit={sendMessage} className="absolute bottom-0 left-0 right-0 mb-1 flex px-2 ">
            <TextInput
              ...
              className="border-grey mb-0 w-full rounded-l-lg border p-2 text-sm font-normal text-gray-800 focus:outline-none"
