  ...
  return (
    <>
      ...
      <div className="flex flex-col">
        ...
        <table className="border-grey flex-no-wrap flex w-full table-auto flex-row overflow-hidden border text-sm xl:text-base text-blue-900 dark:text-blue-200 sm:inline-table">
          {orderTypes > 0 ? (
            <>
              <thead className="border-grey border-b text-sm xl:text-base uppercase text-gray-700 sm:[&>*:not(:first-child)]:hidden">
                {orders.map(() => (
                  <tr
                    key={uuidv4()}
                    className="mb-1 flex flex-col flex-nowrap bg-sky-500 text-blue-900 dark:text-blue-200 sm:mb-0 sm:table-row md:table-row lg:bg-transparent lg:text-blue-900 dark:lg:text-blue-300"
                  >
                    <th className="p-3 text-center w-auto"></th>
                    <th className="p-3 text-left w-auto border-r">Buyer</th>
                    <th className="p-3 text--center border-r">Gig</th>
                    <th className="p-3 text-center  border-r">{type === 'cancelled' ? 'Cancelled On' : 'Due On'}</th>
                    {type === 'completed' && <th className="p-3 text-center border-r">Delivered At</th>}
              ...
              <tbody className="flex-1 sm:flex-none">
                {orders.map((order: IOrderDocument) => (
                  <tr key={uuidv4()} className="bg-blue-500/40 border-b border-grey flex flex-col flex-nowrap sm:table-row mb-2 sm:mb-0 ">
                    <td></td>
                    <td className="flex justify-start gap-3 px-3 py-3 sm:justify-center md:justify-start border-r">
                    ...
                    <td className="p-3 text-left lg:text-center w-[300px] border-r">
                      <div className="grid">
                        <Link
                    ...
                    <td className="p-3 text-left lg:text-center border-r">
                      {type === 'cancelled'
                    ...
                    {type === 'completed' && order.events.orderDelivered && (
                      <td className="p-3 text-left lg:text-center border-r">{TimeAgo.dayMonthYear(`${order.events.orderDelivered}`)}</td>
                    )}
                    <td className="p-3 text-left lg:text-center border-r">${order.price}</td>
                    <td className="px-3 py-1 lg:p-3 text-left lg:text-center border-r">
                      <span
            ...
            <tbody>
              <tr>
                <td className="w-full px-4 py-2 text-sm  text-slate-700 dark:text-slate-400 lg:text-base">No {type} orders to show.</td>
              </tr>
            </tbody>
          )}
        </table>
