...
const LanguageFields: FC<ILanguageEditFieldsProps> = ({
  ...
  const [level, setLevel] = useState<string>(selectedLanguage ? `${selectedLanguage.level}` : 'Basic');
  ...
  return (
    <div className="flex w-full flex-col">
      <div className="mb-6 grid grid-cols-1 gap-y-3 px-3 md:grid-cols-2 md:gap-x-2">
        <div className="">
          <TextInput
            className="border-grey w-full rounded border px-2 py-2 text-sm xl:text-base font-normal text-gray-800 focus:outline-none"
        ...
        <div className="relative">
          <Dropdown
            text={level}
            maxHeight="300"
            mainClassNames="absolute bg-white z-50 text-sm xl:text-base"
      ...
      <div className="z-20 my-4 mt-10 flex cursor-pointer justify-center md:z-0 md:mt-0">
        <Button
          ...
          className={`rounded bg-sky-500 px-6 py-1 text-center text-sm xl:text-base font-bold text-white hover:bg-sky-400 focus:outline-none md:py-2
        ...
        &nbsp;&nbsp;
        <Button
          className="rounded bg-rose-600/40 px-6 py-1 text-center text-sm xl:text-base font-bold hover:bg-rose-600/80 focus:outline-none md:py-2"
          label="Cancel"
          onClick={() => {
