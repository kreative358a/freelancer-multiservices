...
const ProfileHeader: FC<IProfileHeaderProps> = ({ sellerProfile, showHeaderInfo, showEditIcons, setSellerProfile }): ReactElement => {
  ...
  return (
    <>
      {showHeaderInfo && (
        <div className="relative flex h-56 flex-col gap-x-4 gap-y-3 bg-white/10 px-6 py-4 md:h-52 md:flex-row">
          <div className="flex h-20 w-20 justify-center self-center md:h-24 md:w-24 lg:h-36 lg:w-36">
            <LazyLoadImage
              // src={
              //   sellerProfile?.profilePicture !== null ? sellerProfile?.profilePicture : 'https://placehold.co/330x220?text=Profile+Image'
              // }
            ...
            <div className="flex cursor-pointer self-center md:block md:self-start">
              ...
              {showItemEdit.fullname && (
                <div className="flex gap-x-4">
                  <TextInput
                    className="mt-2 flex h-7 w-full items-center rounded border border-gray-300 p-2 text-sm font-normal text-gray-800 focus:border focus:border-sky-500/50 focus:outline-none lg:h-9"
