...
const GigsIndexDisplay: FC<IGigsProps> = ({ type }): ReactElement => {
  ...
  return (
    <div className="flex w-screen flex-col">
      ...
      <div className="relative m-auto mb-10 mt-8 min-h-screen w-screen px-6 xl:container md:px-10 lg:px-6 xl:px-4">
        {isLoading && !isSuccess ? (
          <CircularPageLoader />
        ) : (
          <>
            {!isLoading && gigs.length > 0 ? (
              <>
                <h3 className="mb-5 flex gap-3 text-4xl text-blue-950 dark:text-blue-300">
                  {type === 'search' && <span className="">Results for</span>}
                  <strong className="">{gigCategories}</strong>
                </h3>
                <div className="my-5">
