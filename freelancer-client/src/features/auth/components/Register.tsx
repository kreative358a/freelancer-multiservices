...
// import { useDeviceData, useMobileOrientation } from 'react-device-detect'; // l.649
...
const RegisterModal: FC<IModalBgProps> = ({ onClose, onToggle }): ReactElement => {
  // const mobileOrientation = useMobileOrientation(); // l.649
  // const deviceData = useDeviceData(window.navigator.userAgent); // l.649
  // console.log('mobileOrientation: ', mobileOrientation);
  // console.log('deviceData, deviceData');
  const [step, setStep] = useState<number>(1);
  const [alertMessage, setAlertMessage] = useState<string>('');
  const [country, setCountry] = useState<string>('Select Country');
  const [passwordType, setPasswordType] = useState<string>('password');
  const [profileImage, setProfileImage] = useState<string>(avatar); // https://placehold.co/
  ...
  const [userInfo, setUserInfo] = useState<ISignUpPayload>({
    ...
    country: '',
    profilePicture: ''
    // browserName: deviceData.browser.name, // l. 649
    // deviceType: mobileOrientation.isLandscape ? 'browser' : 'mobile' // l. 649
  });
  ...
  const handleFileChange = async (event: ChangeEvent): Promise<void> => {
    ...
    if (target.files) {
      ...
      if (isValid) {
        ...
        // setUserInfo({ ...userInfo, profilePicture: `${dataImage ? dataImage : 'https://placehold.co/330x220?text=Profile+Image'}` });
      }
  ...
  const onRegisterUser = async (): Promise<void> => {
    try {
      ...
      if (isValid) {
        ...
        console.log('onRegisterUser result: ', result);
        setAlertMessage('');
  ...
  return (
    <ModalBg>
      <div className="relative top-[10%] mx-auto w-11/12 max-w-md rounded bg-slate-200 dark:bg-slate-800 md:w-2/3">
        <div className="relative px-5 py-5">
          <div className="flex justify-between text-2xl font-bold text-gray-600">
            ...
            <h1 className="flex w-full justify-center text-blue-950 dark:text-blue-300">Join Freelancer</h1>
            <Button
              className="cursor-pointer rounded text-gray-400 hover:text-gray-600"
              role="button"
              onClick={onClose}
        ...
        {step === 1 && (
          <div className="relative px-5 py-5">
            <div>
              <label
                htmlFor="username"
                className="text-base lg:text-lg font-bold dark:font-semibold leading-tight tracking-normal text-blue-950 dark:text-blue-400"
              >
                Username
              </label>
              <TextInput
                ...
                name="username"
                type="text"
                value={userInfo.username.replace(' ', '')}
                // value={userInfo.username}
                className="mb-5 mt-2 flex h-10 w-full items-center rounded border border-gray-300 pl-3 text-base font-normal text-gray-800 focus:border focus:border-sky-500/50 focus:outline-none"
              ...
              <div className="relative mb-5 mt-2">
                <div className="absolute right-0 flex h-full cursor-pointer items-center pr-3 text-gray-600">
                  {passwordType === 'password' ? (
                    <FaEyeSlash onClick={() => setPasswordType('text')} className="icon icon-tabler icon-tabler-info-circle text-lg" />
                  ) : (
                    <FaEye onClick={() => setPasswordType('password')} className="icon icon-tabler icon-tabler-info-circle text-lg" />
                  )}
                </div>
                <TextInput
                  id="password"
                  ...
                  className="flex h-10 w-full items-center rounded border border-gray-300 pl-3 text-base font-normal text-gray-800 focus:border focus:border-sky-500/50 focus:outline-none"
        ...
        {step === 2 && (
          <div className="relative px-5 py-5">
            <div className="h-24">
              ...
              <div id="country" className="relative mb-5 mt-2 bg-slate-200/80 dark:bg-slate-800/80 ">
                <Dropdown
                  text={country}
                  maxHeight="200"
                  mainClassNames="absolute bg-transparent text-sm xl:text-base text-blue-950 dark:text-blue-200 font-semibold z-50"
              ...
              >
                {profileImage && (
                  <img
                    ...
                    src={profileImage}
                    alt="Profile Picture"
                    className="left-0 top-0 h-20 w-20 rounded-full bg-white/60 object-cover"
                  />
                )}
                {!profileImage && (
                  <div className="left-0 top-0 flex h-20 w-20 cursor-pointer justify-center rounded-full bg-white/90"></div>
                )}
                {showImageSelect && (
                  <div
                    onClick={() => fileInputRef.current?.click()}
                    className="absolute left-0 top-0 flex h-20 w-20 cursor-pointer justify-center rounded-full bg-white/90"
                  >
            ...
            <Button
              ...
              className={`text-md block w-full cursor-pointer rounded bg-sky-500 px-8 py-2 text-center font-bold text-white hover:bg-sky-400 focus:outline-none ${
                !userInfo.country ? 'cursor-not-allowed' : 'cursor-pointer'
              }`}
              label={`${isLoading ? 'SIGNUP IN PROGRESS...' : 'SIGNUP'}`}
              onClick={onRegisterUser}
            />
        ...
        <div className="px-5 py-4">
          <div className="ml-2 flex w-full justify-center text-base font-medium">
