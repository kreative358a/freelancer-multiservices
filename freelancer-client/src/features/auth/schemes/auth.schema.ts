...
const loginUserSchema: ObjectSchema<ISignInPayload> = object({
  ...
  // browserName: string().optional().nullable().notRequired(), // l. 647
  // deviceType: string().optional().nullable().notRequired() // l. 647
});
...
const registerUserSchema: ObjectSchema<ISignUpPayload> = object({
  ...
  // profilePicture: string().optional().nullable().notRequired()
