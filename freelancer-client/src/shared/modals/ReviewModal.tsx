      ...
      <div className="fixed bottom-0 left-0 right-0 top-0 flex items-center justify-center">
        {isLoading && (
          <div className="fixed bottom-0 left-0 right-0 top-0 z-[50] flex w-full items-center justify-center opacity-80">
            <div className="absolute bottom-auto left-auto right-auto top-auto flex min-h-[290px] min-w-[500px] flex-col items-center justify-center bg-slate-200/60 dark:bg-slate-800/60 backdrop-blur p-4">
              <FaCircleNotch className="animate-spin" size={40} color="#50b5ff" />
              <span>Adding...</span>
            </div>
          </div>
        )}
        <div className="rounded relative bottom-auto left-auto right-auto top-auto max-h-[90vh] min-w-[500px] p-4 bg-slate-200/60 dark:bg-slate-800/60 backdrop-blur border-2 border-gray-500/60">
          ...
          <div className="flex justify-end gap-3">
            ...
            <Button
              className={`rounded px-6 py-3 text-center text-sm font-bold text-white focus:outline-none md:px-4 md:py-2 md:text-base ${
                !review || reviewRating === 0 ? 'cursor-not-allowed bg-sky-200' : 'bg-sky-500/90 hover:bg-sky-500'
              }`}
