  ...
  const sendGigOffer = async (): Promise<void> => {
    try {
      const messageBody: IMessageDocument = {
        ...
        // senderPicture: `${authUser?.profilePicture}` || 'https://placehold.co/330x220?text=Profile+Image',
        // senderPicture: `${authUser?.profilePicture !== null ? authUser?.profilePicture : 'https://placehold.co/330x220?text=Profile+Image'}`,
        senderPicture: `${authUser?.profilePicture !== null && authUser?.profilePicture !== 'undefined' ? authUser?.profilePicture : 'https://placehold.co/330x220?text=Profile+Image'}`,
        receiverUsername: receiver?.username,
        receiverPicture: `${receiver?.profilePicture !== null && receiver?.profilePicture !== 'undefined' ? receiver?.profilePicture : 'https://placehold.co/330x220?text=Profile+Image'}`,
        isRead: false,
        hasOffer: true,
        offer: {
          gigTitle: `${gigTitle}`,
      ...
      console.log('sendGigOffer error: ', error);
  ...
  return (
    <ModalBg>
      <div className="fixed bottom-0 left-0 right-0 top-0 z-[30] flex w-full items-center justify-center">
        <div className="rounded relative bottom-auto left-auto right-auto top-auto max-h-[90vh] min-w-[500px] bg-slate-200/60 dark:bg-slate-800/60 backdrop-blur p-4 border-2 border-gray-500/60">
          <div className="relative px-5 py-5">
            <Button
              onClick={cancelBtnHandler}
              className="absolute cursor-pointer rounded text-gray-400 text-2xl hover:text-gray-600 right-0 top-0"
              role="button"
          ...
          <div className="relative mb-16 px-5">
            ...
            <div>
              <label htmlFor="description" className="text-sm xl:text-base font-semibold leading-tight tracking-normal">
                Description<sup className="top-[-0.1em] text-base text-red-500">*</sup>
              </label>
              <TextAreaInput
                className="border-grey mt-2 mb-1 w-full rounded border p-2 text-sm xl:text-base  font-normal text-gray-900 focus:outline-none"
            ...
            <div className="mt-2">
              <label htmlFor="price" className="text-sm xl:text-base font-semibold leading-tight tracking-normal">
              ...
              <div className="relative mb-5 mt-2">
                <TextInput
                  ...
                  type="number"
                  value={offer.price}
                  className="flex h-10 w-full items-center rounded border border-gray-300 pl-3 text-sm xl:text-base font-normal text-gray-900 focus:border focus:border-sky-500/50 focus:outline-none"
            ...
            <div className="mb-6">
              <label htmlFor="country" className="text-sm xl:text-base font-semibold leading-tight tracking-normal">
              ...
              <div id="country" className="relative mb-5 mt-2">
                <Dropdown
                  text={offer.delivery}
                  maxHeight="200"
                  mainClassNames="absolute bg-white z-50 text-gray-900 text-sm xl:text-base"
