      ...
      <ul className="flex gap-4">
        ...
        {showNumbers &&
          paginationCount.map((_, index: number) => (
            <li
              key={uuidv4()}
              className={`cursor-pointer text-base xl:text-lg px-2 py-2 xl:py-1 ${itemOffset === index + 1 ? 'border-b-2 font-bold ' : ''}`}
              onClick={() => {
        ...
        <div
          className={`p-3 ${
            itemOffset === paginationCount.length
              ...
              : 'rounded-full border border-sky-400 cursor-pointer '
          }`}
          onClick={() => {
