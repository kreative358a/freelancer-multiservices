...
const HomeHeaderSideBar: FC<IHeaderSideBarProps> = ({ setOpenSidebar }): ReactElement => {
  ...
  const settings: ISettings[] = [
    ...
    { id: 4, name: 'Settings', url: `/${lowerCase(`${seller?.username ? seller?.username : buyer?.username}`)}/edit`, show: true }
    // { id: 4, name: 'Settings Buyer', url: `/${lowerCase(`${buyer?.username}`)}/edit`, show: true }
  ];
  ...
  return (
    <div
      className={'fixed left-0 top-0 z-[150] flex h-screen w-full bg-black/40 transition-all duration-500 backdrop-blur-sm'}
      onClick={() => {
        if (setOpenSidebar) {
      ...
      >
        <div className="z-2 sticky top-0 flex w-full flex-col items-start justify-start gap-6 bg-transparent">
          ...
          <div
            ...
            className="cursor-pointer text-base font-medium text-blue-600 dark:text-blue-400"
          >
          ...
          <div className="flex w-full cursor-pointer flex-col text-base font-medium text-slate-800 dark:text-slate-200 hover:text-blue-600">
