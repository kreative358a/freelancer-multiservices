...
const HeaderSideBar: FC<IHeaderSideBarProps> = ({ setShowRegisterModal, setShowLoginModal, setOpenSidebar }): ReactElement => {
  ...
  return (
    <div
      className={'fixed left-0 top-0 z-40 h-screen w-full bg-black/40 backdrop-blur-sm transition-all duration-500 flex'}
      onClick={() => {
    ...
    >
      <div
        className={`absolute top-0 z-20 flex h-screen w-[280px] flex-col items-start justify-start gap-4 bg-slate-200/80 dark:bg-slate-700/80 backdrop-blur-sm p-6 ${
      ...
      >
        <div className="z-2 sticky top-0 flex w-full flex-col items-start justify-start gap-6 bg-transparent">
          ...
          <div className="cursor-pointer text-base font-medium flex flex-col w-full text-slate-800 dark:text-slate-200 hover:text-blue-600">
