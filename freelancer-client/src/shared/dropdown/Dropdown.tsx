  ...
  return (
    <div className={`w-full divide-y divide-gray-100/0 rounded border ${mainClassNames}`} style={style}>
      {(!showSearchInput || showSearchInput) && !toggleDropdown && (
        <Button
          className="bg-teal flex w-full justify-between rounded px-3 pt-1 pb-1.5 text-slate-800 text-sm xl:text-base"
          label={
            <>
              <span className="truncate font-semibold  dark:text-slate-500 dark text-sm xl:text-base">{text}</span>
              {!toggleDropdown ? (
                <FaChevronDown className="float-right mt-1 h-4 fill-current text-slate-800 dark:text-slate-100 hover:text-slate-800 text-base xl:text-lg " />
              ) : (
                <FaChevronUp className="float-right mt-1 h-4 fill-current text-slate-800 dark:text-slate-100 hover:text-slate-800 text-base xl:text-lg" />
              )}
            </>
          }
      ...
      {showSearchInput && toggleDropdown && (
        <div className="flex bg-slate-200/80 dark:bg-slate-800/80 text-blue-950 dark:text-blue-200">
          <TextInput
            ...
            name="search"
            value={inputText}
            className="h-10 w-full items-center rounded pl-3 text-sm font-semibold focus:outline-none xl:text-base bg-transparent text-blue-950 dark:text-blue-200"
            placeholder="Search..."
          ...
          >
            <p className="mx-3 text-sm xl:text-base fill-current text-blue-950/80 dark:text-blue-200/80 hover:text-blue-950 dark:hover:text-blue-200">
              clear
            </p>
          </div>
